...
*rm -r .git* - **удаление локального репо**

*git stash* - **временное сохранение незакоммиченных изменений**

*git tag* - **создание тэга, постоянной метки**

*git fetch >url* - **вытягивает из удалённого репо все новые изменения и сохраняет локально**

*git remote* - **выводит список удалённых подключений к другим репо**
* *-v* - **тжс но ещё и с URL**
* *add _name >url* - **создание нового подключения к удалённом репо и добавляет имя для убоства**
* *rm >name* - **удаление подключения к удалённом урепо**
