...
2. [Игнорирование накопившихся файлов](#полезно-знать) 
...
Добавляет содержимое рабочего каталога в индекс (staging area) для последующего коммита (фиксации изменений).
    ...
    ***ВАЖНО!*** *После слова "add" необходимо указать наименование Вашего файла c расширением. (Чтобы не вводить имя файла целиком, достаточно ввести первые три символа **с учетом регистра** и нажать "Tab", программа сама распознает имя Вашего файла). Пример:* ![Add](/Add.png)
...
5. **git checkout**  
Используется для перехода от одного коммита к другому с обязтаелным указанием первых четырех символов хеш-кода после слова "checkout". Пример: ![Checkout](/Checkout.png)
    + **git checkout master**  
...
1. **git clone**  
Команда для клонирования репозитория из удаленного источника
...
4. **git remote add origin <ссылка>**  
...
![failes_to_ignore](/files_to_ignore.png)
